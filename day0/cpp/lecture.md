# C++ 基础 · 自学讲义
<!-- ⚠️ 这个文件由 lab/sync_grader.py 从 lab/day0/cpp/lecture.md 复制而来。
...
> 那时候 `sizeof` 拿到的是指针大小，不是数组大小 —— 见 §7b.5。
...
这一节是 **Day 3 CAN 课的前置**。CAN 课讲协议（字节序、字段含义、ID 规则），
这里讲**表达手段**（怎么把 8 个字节拆开、怎么把两字节拼成一个数）。
两边不重复。
...
注意最后那个 `model_` —— 它是一个**函数指针**，也可以放进初始化列表。
下一节 §7b.4 专门讲它。
...
**你只需要能读懂，不要求写。** 注意捕获了外部变量的 lambda 不能赋给裸函数指针
（它不再是无状态的函数了，§7b.6）。
